...
course_reference_numbers = list(pandas_gbq.read_gbq(sql, project_id=PROJECT_ID)['course_reference_number'])
...
def fetch_course_details(course_reference_number):
    ...
    headers["Referer"] = f"https://www.myskillsfuture.gov.sg/content/portal/en/training-exchange/course-directory/course-detail.html?courseReferenceNumber={course_reference_number}"
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to retrieve data for course {course_reference_number}: {response.status_code}")
        return None
...
def parse_mode_of_trainings(course_detail_dict):
    ...
    return [
        ModeOfTraining(
            ...
            mode_of_training_description=training_mode.get('description', '') if isinstance(training_mode, dict) else ''
        )
    ...
    for course_run in course_runs:
        ...
        for trainer in link_course_run_trainer:
            ...
            trainer_instance = {
                ...
                'trainer_qualification_level': trainer_dict.get('qualificationLevel', ''),
...
def parse_job_roles(course_detail_dict):
    ...
    return [
        JobRoleCourseDetails(course_reference_number=course_reference_number, job_role=role)
...
def parse_course_details(course_detail_dict):
    ...
    course_details = CourseDetails(
        ...
        qualification_attained_id=qualification_attained.get('qualificationAttainedCode', ''),
...
def get_all_courses_data(course_reference_numbers):
    ...
    for idx, course_reference in enumerate(course_reference_numbers):
        ...
        if courses_processed > 1:  # Avoid division by zero for the first iteration
            ...
            estimated_completion_time = start_time + (average_time_per_course * total_courses)
            
            # Convert to human-readable format (minutes)
            estimated_time_remaining_minutes = estimated_time_remaining / 60
            estimated_completion_time_readable = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(estimated_completion_time))

            print(f"Estimated time remaining: {estimated_time_remaining_minutes:.2f} minutes,\tcomplete at {estimated_completion_time_readable}")
        else:
    ...
    mode_of_trainings_df = pd.DataFrame([mode.__dict__ for mode in all_mode_of_trainings])
    course_runs_df = pd.DataFrame(all_course_runs)

    return course_details_df, trainers_df, job_roles_df, mode_of_trainings_df, course_runs_df


for course_references in course_reference_numbers_list:

    course_details_df, trainers_df, job_roles_df, mode_of_trainings_df, course_runs_df = get_all_courses_data(course_references)
